...
from torch import nn, Tensor, broadcast_tensors
from torch.amp import autocast

from models.NativeSparseAttention.utils import apply_rotary_emb
...
class RotaryEmbedding(nn.Module):
    def __init__(
        ...
        assert (custom_freqs is not None) or (
            freqs_for in ["lang", "pixel", "constant"]
        ), "Improper freq values"
    ...
    def rotate_queries_or_keys(self, t, seq_dim=None, offset=0, scale=None):
        ...
        assert (not self.use_xpos) or (scale is not None), (
            ...
            "for rotary embeddings that can be length-extrapolated"
        )
        ...
        return apply_rotary_emb(
            freqs, t, scale=scale if scale is not None else 1.0, seq_dim=seq_dim
        )
    ...
    def rotate_queries_with_cached_keys(self, q, k, seq_dim=None, offset=0):
        dtype, device, seq_dim = (
            q.dtype,
            q.device,
            seq_dim if seq_dim is not None else self.default_seq_dim,
        )
