...
def add_common_args(parser):
    ...
    parser.add_argument(
        "--only-cpu", action="store_true", help="Perform evaluation on CPU"
    )
    parser.add_argument(
        "--throughput", action="store_true", help="Test throughput only"
    )
    parser.add_argument(
        "--use-sync-bn", action="store_true", default=False, help="sync bn"
    )
    parser.add_argument(
        "--use-wandb",
        action="store_true",
        ...
        if ckpt_head_bias.shape != now_model_bias.shape:
            ...
            if len(ckpt_head_bias) == 21841 and len(now_model_bias) == num_classes:
                ...
                with open(fname) as fin:
                    mapping = torch.Tensor(list(map(int, fin.readlines()))).to(
                        torch.long
                    )
                for name in mnames:
                    v = params[name]
    ...
    if Nc1 != Nc2:
        if Nc1 == 21841 and Nc2 == 1000:
            ...
            with open(map22kto1k_path) as fin:
                mapping = torch.Tensor(list(map(int, fin.readlines()))).to(torch.long)
